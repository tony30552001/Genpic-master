...
openwiki:
  ...
  symbols: [App, ProtectedRoute, LoginPage, ResponsiveGoogleLogin, LoginLoadingState, LoginShaderBackground, LibraryPage, AssetCenter, normalizeViewMode, AuthProvider, MotionProvider, ThemeToggle, getTheme, setTheme, getAuthSession, loginWithMicrosoft, loginWithGoogle, requestWithRetry, setCsrfToken]
  test_paths: [src/pages/__tests__/LoginPage.test.jsx, src/services/__tests__/authService.test.js, src/services/__tests__/apiClient.test.js, src/components/common/__tests__/ThemeToggle.test.jsx]
  invariants: ["The browser holds the CSRF token only in memory and sends it on unsafe requests.", "Authenticated requests include the server-issued session cookie rather than a provider token.", "The login surface remains light even when the application root is dark, and its optional shader never runs for reduced-motion users or without usable WebGL2.", "The root theme is applied before first paint and an explicit stored choice overrides system preference."]
  validation_commands: [pnpm test --run src/pages/__tests__/LoginPage.test.jsx src/context/__tests__/AuthContext.test.jsx src/services/__tests__/authService.test.js src/services/__tests__/apiClient.test.js, pnpm test --run src/components/common/__tests__/ThemeToggle.test.jsx]
---
...
`LoginPage` is presentation and recovery UI; it does not own a second authentication flow. It redirects an already authenticated visitor to the protected route recorded in router state (or `/`), renders `LoginLoadingState` while `useAuth` bootstraps the BFF session, and then delegates Microsoft and Google success to `handleMicrosoftLogin` and `handleGoogleLoginSuccess`. A Google-widget failure is page-local state; it is displayed alongside `profileError` or session expiry, without changing the server session contract.
