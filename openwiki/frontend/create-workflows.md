...
For a studio/hook or timeline contract change, begin with `PptMasterStudio.jsx`, `DeckSetupSummary.jsx`, `usePptMasterDeck.js`, `DeckProgress.jsx`, `DeckTimeline.jsx`, `deckSteps.js`, `pptTemplateCopy.js`, then the `aiService` methods (`listPptTemplates`, `createDeckJob`, `getDeckJob`, `waitForDeckJob`, `downloadDeckJobPptx`) and [PPT Master deck jobs](../backend/ppt-master-decks.md). `src/components/create/__tests__/PptMasterStudio.test.jsx` has retrievable cases for the full idle form, collapsed generating setup, explicit setup re-opening while progress remains visible, and collapsed completed setup; run it first for studio composition or progress-title changes:

```sh
pnpm test --run src/components/create/__tests__/PptMasterStudio.test.jsx
```

`src/hooks/__tests__/usePptMasterDeck.test.jsx` has retrievable cases for restoring running/succeeded jobs, clearing a missing/failed job, keeping an ID after a connection failure, and stop tracking; `src/services/__tests__/aiService.test.js` covers transient polling, immediate 404 handling, and exhaustion after five failures. `deckSteps.test.js` verifies all-pending initialization, newest event state, ID ordering, nested slide grouping, skipped steps, and unknown-step exclusion; `DeckTimeline.test.jsx` verifies all labels, running-step expansion, and collapse. These mocks do not prove authenticated API, Blob upload/download, event persistence, or worker behavior. Run the focused lifecycle and timeline check when the hook, service, event reducer, or timeline changes:
