...
openwiki:
  ...
  source_paths: [src/InfographicGenerator.jsx, src/components/create/DocumentUploader.jsx, src/components/create/DocumentScenes.jsx, src/services/aiService.js, src/services/apiClient.js, src/utils/pptxExport.js, src/components/create/ImagePreview.jsx, src/components/create/ImageGeneratingState.jsx, src/hooks/useDocumentAnalysis.js]
  symbols: [AnalysisProgress, ImageGeneratingState, handleGenerateScene, handleApplyDocumentStyle, handleClearDocumentStyle, exportToPptx, exportWithAutomizer, generatePresentationPptx, apiPostBlob, addPptxTable, addPptxChart, getPptxScenes, getPptxTables, getPptxCharts, normalizePptxTable, normalizePptxChart, extractPptxBullets, sanitizePptxFilename]
  ...
  invariants: [Document scene generation uses the AI recommendation unless a saved-style override is active., General and document-scene generation states use the same visual feedback component without sharing generation state., PPTX export includes each valid analyzed scene whether or not it has a generated image., Valid native table or chart data takes precedence over the generated image or placeholder in the PPTX visual region., Server export accepts only base64 image data URLs after browser-side image conversion.]
  validation_commands: [pnpm test --run src/utils/__tests__/pptxExport.test.js src/services/__tests__/aiService.test.js src/services/__tests__/apiClient.test.js]
---
...
`InfographicGenerator` owns the document-mode tab before it renders `DocumentUploader`: **Document analysis** fixes the child mode to `storyboard`; **Presentation generation** fixes it to `presentation`. A result remains visible only while its `analysis_mode` matches the selected tab, so changing modes does not reinterpret a prior result or expose its batch image-generation bar. `DocumentUploader` permits pasted outlines only in presentation mode and switches an open outline tab back to file input when the parent mode changes.
...
Image, PDF, and browser-generated PowerPoint output are browser artifacts, not API records. `DocumentScenes.jsx::exportToPptx()` passes valid object scenes through `src/utils/pptxExport.js::getPptxScenes`, creates a 16:9 deck with one slide per returned scene, and renders scene number/title, editable bullet text, and optional speaker notes. The implementation can include image-less analyzed slides, but the current UI shows both the local PPTX and PDF actions only after at least one scene has a generated image (`generatedCount > 0`). The separate Automizer action is shown for any nonempty scene list. Thus, a missing `generatedImage` does not exclude an analyzed slide after an export path is available. `extractPptxBullets` trims and drops empty/null bullet values, then falls back to a nonempty `scene_description`; `sanitizePptxFilename` permits word/CJK/space/hyphen characters with a `presentation` fallback.
...
`exportWithAutomizer` is a separate server-export path. It uses the same `getPptxScenes` selection and browser `fetchImageAsBase64` conversion, replaces every available `generatedImage` with its data URL or `null`, then calls `aiService.generatePresentationPptx`. That service delegates to credentialed, CSRF-protected `apiClient.apiPostBlob`, receives a `Blob`, triggers a `-automizer-<timestamp>.pptx` download, and revokes its object URL. A failed image conversion does not prevent export but is reported after download. This route deliberately receives embedded image data rather than SAS/Blob URLs; its authenticated handler and Automizer rendering boundary are documented in [AI generation](../backend/ai-generation.md). It is not a fallback for the browser exporter: the UI disables PDF, local PPTX, and Automizer export while any one is active, and the current Automizer output has no dynamic speaker-note integration.

`api/_shared/__tests__/presentationSchema.test.js` is the corresponding server-contract check; run it with the frontend helper test when changing a shared presentation limit, alias, or fallback policy. `src/services/__tests__/aiService.test.js` verifies the binary-service delegation and `apiClient.test.js` verifies blob parsing through the normal credential/CSRF client. Browser-test an actual export only after changing deck layout, dynamic loading, image embedding, native PPTX rendering, or the Automizer handoff; test an image-less scene, an image-bearing scene, a table-only scene, a chart-only scene, and a scene with both visuals. `generationProgress.test.js` validates time/model progress phases, and service tests validate request contracts.