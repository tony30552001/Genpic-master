...
`InfographicGenerator` owns one `imageQuality` state value, initialized from `DEFAULT_IMAGE_QUALITY` (`medium`), and passes it to `GenerateBar`, `useImageGeneration`, and `useImageTransform`. `src/config.js::IMAGE_MODEL_OPTIONS` contains only `gpt-image-2`, whose fixed `low`/`medium`/`high` quality control is visible in `GenerateBar`. There is no alternate renderer or browser model selector; quality is rendering effort, not a resolution alias.
