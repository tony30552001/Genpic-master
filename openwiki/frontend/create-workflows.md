...
`PptTemplatePicker` may show static SVG samples from `templatePreviewManifest.js`. A template with no manifest entry, or an image that fails to load, still renders as a text-only selectable card. Samples demonstrate color, type, and decorative vocabulary—not deterministic page geometry—because production pages are independently authored. The generated manifest and `public/template-previews/` assets are a derived browser surface; their conditional generation and validation are documented in [development, migrations, and deployment](../operations/development-deployment.md). The API registry is documented in [HTTP API](../backend/http-api.md).
