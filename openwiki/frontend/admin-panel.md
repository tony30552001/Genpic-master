...
openwiki:
  ...
  source_paths: [src/App.jsx, src/pages/AdminPage.jsx, src/components/admin/AdminPanel.jsx, src/components/admin/UserFilterSelect.jsx, src/components/admin/LlmModelSettings.jsx, src/components/admin/ImageModelSettings.jsx, src/components/common/ImageLightbox.jsx, src/services/adminService.js, api/admin/index.js, api/_shared/historySource.js]
  symbols: [ProtectedAdminRoute, AdminPage, AdminPanel, USER_SEARCH_DEBOUNCE_MS, HISTORY_SOURCE_FILTERS, historySourceLabel, UserFilterSelect, AdminRemoteImage, loadAdminImage, ImageModelSettings, LlmModelSettings, listAdminImageModels, createAdminImageModel, testAdminImageModel, listAdminUsers, listAdminHistory, listAdminLlmModels, assignAdminLlmRole, getAdminHistoryImage, getAdminStylePreview, ImageLightbox, viewableHistoryItems]
  test_paths: [src/__tests__/App.test.jsx, src/components/admin/__tests__/AdminPanelSectionLoading.test.jsx, src/components/admin/__tests__/AdminHistoryPreview.test.jsx, src/components/admin/__tests__/LlmModelSettings.test.jsx, src/components/admin/__tests__/ImageModelSettings.test.jsx, src/components/common/__tests__/ImageLightbox.test.jsx, api/admin/__tests__/adminResources.test.js]
  invariants: ["ProtectedAdminRoute shows loading exactly for isLoading or for an authenticated session with neither profile nor profileError; it does not read isProfileLoading, so an initial profile error stops loading and cannot grant admin access.", "An already authorized admin remains mounted during explicit profile refresh; a refreshed non-admin profile redirects to creation, and session initialization or identity changes clear old authorization.", The panel loads only the initially active users section and loads each other section once when opened., User-list keyword input is debounced and pagination reuses the applied query., A user or history-source filter invalidates and reloads only the currently active history or styles section., Provider selectors are mutually exclusive because they share one selected user ID and filter already-loaded user options locally., The history-source filter passes only general document image-transform or unknown to the server and unknown represents records whose source is null., History preview navigation includes only image-bearing records loaded in the current page., List responses expose availability flags rather than image URLs; image URLs are retrieved through tenant-scoped admin endpoints and cached in-browser by record ID., Closing a shared lightbox restores prior focus and document scrolling., Browser model settings never receive an analysis API key., Catalog and policy controls are separate cards in the same models section; either successful write refreshes profile metadata.]
  validation_commands: [pnpm test --run src/__tests__/App.test.jsx, pnpm test --run api/admin/__tests__/adminResources.test.js src/components/admin/__tests__/AdminPanelSectionLoading.test.jsx src/components/admin/__tests__/AdminHistoryPreview.test.jsx src/components/admin/__tests__/LlmModelSettings.test.jsx src/components/common/__tests__/ImageLightbox.test.jsx]
---

# Administrator panel and history preview

`/admin` is the administrator browser workspace. `AdminPage` is a thin wrapper around `AdminPanel`; `ProtectedAdminRoute` in [browser application and authentication](application.md) shows loading exactly for `isLoading || (isAuthenticated && !profile && !profileError)`, then redirects non-admin users to `/`. It does not read `isProfileLoading`, so it also gates the initial profile timer gap before that flag becomes true; an initial profile error stops loading and supplies no admin access. During an explicit refresh, a previously authorized admin keeps the mounted panel while the profile request is pending or fails, so panel-local selected model section and saved/refresh-failed notifications survive. A refreshed non-admin profile redirects to creation after the refresh completes. On mount, the panel loads only its active **users** section. Opening history, styles, or models loads that section once and retains it in memory; changing the user filter invalidates history and styles but refetches only the section currently on screen. The users section debounces its keyword field for `USER_SEARCH_DEBOUNCE_MS` (300 ms), sends the trimmed applied value through `listAdminUsers`, and retains that value across its pagination and page-size reloads. `LlmModelSettings` remains the separate composition for encrypted analysis-model settings. The panel also exposes section-specific pagination, user role/status changes, image-model-policy saves, administrator style deletion, and analysis-model CRUD/role assignment. The server-side authorization, tenant scope, last-provider field, literal-safe search, list/image contract, and management endpoints are canonical in [authentication, tenancy, and administration](../backend/auth-tenancy-admin.md).
...
The **models** section contains separate catalog and policy cards: `ImageModelSettings` manages the catalog, while the policy card manages the allowed/default model. They are separate concerns, not separate navigation sections. Catalog create/update/delete and policy saves both call `refreshProfile`, so browser metadata is refreshed after either successful change; a refresh failure is reported separately after a successful write. Creating a catalog record does not alter policy. Editing keeps model key, API type, and deployment read-only; the replacement-key input starts empty and a blank submission retains the encrypted server value. The form gets its six allowed qualities from the server and requires a supported default.

A deliberate **paid generation test** confirms first, submits `POST /management/image-model-tests` for the selected saved key at `low`, then uses `waitForImageJob`. Stopping only aborts local waiting and explicitly does not cancel the remote work; success proves only that low-quality generation completed, not other qualities or edits. The catalog/API and job semantics are documented in [authentication, tenancy, and administration](../backend/auth-tenancy-admin.md) and [AI generation](../backend/ai-generation.md).

For this UI, start at `ImageModelSettings.jsx`, `adminService.js`, `api/admin/imageModels.js`, and `imageModels.js`. `ImageModelSettings.test.jsx` is mocked UI coverage for failed loads, immutable identity, key clearing, quality form validation, returned catalog replacement, paid-test confirmation/polling/cancellation language, and profile-refresh failure. It is not a live Azure test. Run:

```sh
pnpm test --run src/components/admin/__tests__/ImageModelSettings.test.jsx api/admin/__tests__/imageModels.test.js
```
...
Consult this page for `/admin` loading, route-refresh retention, user search/provider/history-source filtering, presentation, history/style thumbnails, preview, or use of the shared lightbox. Start route-authorization changes at `src/App.jsx` (`ProtectedAdminRoute`) and `src/context/AuthContext.jsx` (`refreshProfile`): preserve the exact `isLoading || (isAuthenticated && !profile && !profileError)` loading condition, not `isProfileLoading`. It must cover the initial profile timer gap; an initial profile error must stop loading without granting access. A refresh for an existing admin must not unmount `AdminPanel`; a refreshed viewer must redirect to creation. Then start panel changes at `src/components/admin/AdminPanel.jsx`: `requestedSectionsRef` controls once-per-section initial loads, `loadUsersSection`/`loadHistorySection`/`loadStylesSection`/`loadSettingsSection` own independent fetches, and `runRefresh` owns explicit pagination refreshes. `USER_SEARCH_DEBOUNCE_MS` and `appliedUserSearchRef` form the user-search seam: reset to page one for a changed applied query and preserve the applied query on later user-list pagination. `PROVIDER_FILTERS`, `userFilterGroups`, and `selectedUserId` compose provider grouping; `UserFilterSelect` is deliberately client-side over the loaded options. `HISTORY_SOURCE_FILTERS`, `historySource`, and `handleHistorySourceChange` are the server-backed history-attribution seam; preserve the special null-only `unknown` option from [resources](../backend/resources.md). `AdminRemoteImage`, `loadAdminImage`, and `adminImageCache` are the protected-image seam; keep the list's availability flags separate from image URL retrieval. `previewHistoryId`, `viewableHistoryItems`, `previewIndex`, and `previewDetails` form the preview composition seam. Change `src/components/common/ImageLightbox.jsx` for modal behavior shared with the style library; update [Asset Center](asset-center.md) at the same time if the minimal-preview contract changes. Change `src/services/adminService.js` and [authentication, tenancy, and administration](../backend/auth-tenancy-admin.md) together when the management request contract changes.

Preserve these rules:

1. Route guarding is not a visual concern: retain `ProtectedAdminRoute` and do not make the panel itself the authorization boundary. Its loading condition is exactly `isLoading || (isAuthenticated && !profile && !profileError)`, not `isProfileLoading`: initial authorization—including its timer gap—must show loading, and an initial profile error must stop loading without granting access. Retain children during explicit refresh only when existing profile authorization is admin, and redirect when the replacement profile revokes that role.
...
`App.test.jsx` uses the real `AuthProvider` and `App` with mocked pages/services to verify the route boundary in four cases: initial authorization remains hidden while the profile is pending and a resolved viewer redirects to creation; an authorized panel retains its selected section and saved notification through a successful refresh; it retains the selected section and reports a failed refresh; and a refreshed viewer role redirects to creation. The exact initial-error stop-and-deny behavior follows the route condition in `App.jsx`. Run it alone for route-refresh work. `AdminPanelSectionLoading.test.jsx` verifies mount-only users loading, once-only section loading, active-section filter reload, mutually isolated/searchable Entra/Google user selectors, debounced user-list keyword forwarding, and history-source label/request forwarding. `AdminHistoryPreview.test.jsx` mocks `adminService` and verifies remote image retrieval, details/download, navigation that skips an image-less record, Escape close, and the noninteractive missing-image state. `ImageLightbox.test.jsx` covers the reusable optional-control and keyboard behavior. `api/admin/__tests__/adminResources.test.js` verifies recognized and legacy source filters plus rejection of an unsupported source. Run the focused route-refresh check:

```sh
pnpm test --run src/__tests__/App.test.jsx
```
