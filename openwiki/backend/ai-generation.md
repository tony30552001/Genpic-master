...
`normalizeImagePurpose` accepts `infographic`, `storyboard`, or `freeform` case-insensitively after trimming; all other or absent values fall back to `infographic`. The first two add presentation-slide or cinematic-storyboard composition guidance respectively, while `freeform` adds none. The handler returns the resulting `prompt` for direct work and when it creates a durable job. Browser callers therefore must send inputs, not reproduce this composition or persist a guessed final prompt. `buildGenerationTextDirective` and optimizer-facing `buildImageTextDirective` share `IMAGE_TEXT_LANGUAGES`: supported IDs produce aligned directives, `none` prohibits rendered text, and missing/unknown IDs add nothing.
