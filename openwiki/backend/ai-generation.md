...
`POST /optimize-prompt` resolves the tenant's `prompt_optimization` role and accepts required `userScript`, optional `styleContext`, optional `imageLanguage`, and optional `templateContext`. `POST /optimize-scene` similarly resolves `scene_optimization` and accepts scene fields plus optional style context and `imageLanguage`, but no general-editor template context. Both use `generateJson`, so they share tenant-model selection and retry behavior with the [provider-neutral runtime](#provider-neutral-runtime). They are optimization surfaces, not image-rendering endpoints: they return structured prompt/scene text that the browser later supplies to the generation flow documented in [creation workflows](../frontend/create-workflows.md).
...
For a new image-text language, task-structure field, prompt schema, mode guarantee, or style-analysis reuse rule, change the shared helper or owning prompt constant, both consumer request paths in [creation workflows](../frontend/create-workflows.md), and focused tests together. `templateContext.js` is the sole server authority for context version, allowed template IDs, output-type consistency, and module-count/flow allowlists. It also rejects guidance/pitfall arrays unless each has 1–4 nonempty strings of at most 240 characters and their combined content is no longer than 1,600 characters; accepted rule text is caller-supplied, not server-allowlisted. `normalizeTemplateContext` returns canonical title/purpose instead of trusting client title/purpose; `buildTemplateInstruction` turns that result into the instruction passed to the provider. Invalid values produce `TemplateContextError` with `400 invalid_template_context` before model resolution/calls. `templateContext.test.js` covers pure normalization and instruction construction; `optimize-prompt/__tests__/index.test.js` verifies instruction forwarding and early rejection. `imageTextLanguage.test.js` proves a recognized optimizer mapping, the image-model quoted-text/default-language wording, generation `none`, and unsupported/missing values; `imagePrompt.test.js` proves the transform receives that generation directive. There is still no focused test for optimizer authorization, role resolution, or response fallback, and no focused test covers style analysis or transform mode wording.
...
`POST /generate-images` is the server-owned text-to-image prompt boundary. It requires nonblank `userScript`, rather than accepting a browser-assembled `prompt`; optional `stylePrompt`, array `styleTags`, `purpose`, `imageLanguage`, and `templateContext` remain separate creative inputs. Before touching an input upload or provider, the handler normalizes a supplied context through `templateContext.js`. Its server-derived purpose overrides the separately supplied `purpose`; `buildImagePrompt` adds the resulting module-count, flow, guidance, and pitfall instruction as a separate clause before the topic and composition defaults. The client cannot introduce an unknown template ID, inconsistent output type, unsupported purpose, module count, or information flow; guidance/pitfall text is accepted only within the server's array and length bounds. Omit the field for the legacy/default path.

`normalizeImagePurpose` accepts `infographic`, `storyboard`, or `freeform` case-insensitively after trimming; all other or absent values fall back to `infographic`. Infographic and storyboard add presentation-slide or cinematic-storyboard composition guidance **only unless the description already specifies framing**. `freeform` sends its description untouched by composition or image-text-language system additions, but a valid template context can still add its structural instruction and can resolve the purpose away from freeform. It always retains supplied style prose and palette cues. The handler returns the resulting `prompt` for direct work and when it creates a durable job. Browser callers therefore must send inputs, not reproduce this composition or persist a guessed final prompt. `buildGenerationTextDirective` and optimizer-facing `buildImageTextDirective` share `IMAGE_TEXT_LANGUAGES`: for a supported generation language, quoted literal text remains exactly as authored and only other rendered text defaults to that language; `none` prohibits rendered text; and missing/unknown IDs add nothing.

The pure contract is covered by `templateContext.test.js` and `imagePrompt.test.js`: template normalization/instruction construction, purpose normalization/defaulting, style prose/tag separation and de-duplication, context overriding a separately supplied purpose, conditional infographic/storyboard framing, a freeform description with no system directive, quoted-text/default-language wording, missing content, transform-mode guarantees, and unknown-mode fallback. `generate-images/__tests__/index.test.js` verifies invalid context rejection before generation and provider receipt of the normalized structural instruction; `optimize-prompt/__tests__/index.test.js` does the analogous optimizer checks. `imageTextLanguage.test.js` isolates generation-directive behavior. `aiService.test.js` is the narrow consumer serialization check. There is no returned-prompt, durable-job-forwarding, tenant-policy, or full consumer-composition test; add the relevant layer before changing public validation or persistence behavior.

```sh
pnpm test --run api/_shared/__tests__/templateContext.test.js api/_shared/__tests__/imagePrompt.test.js api/generate-images/__tests__/index.test.js api/optimize-prompt/__tests__/index.test.js src/services/__tests__/aiService.test.js
```
