...
`POST /image-transform` always queues a `gpt-image-2` edit and returns `202`; Gemini transformation remains synchronous. Text generation queues only non-reference GPT work in the standalone runtime; a GPT generation with a reference image, or a Functions-runtime generation, remains direct. `createImageJob` normalizes and stores `quality`; it also records `operation` as `generate` or `edit`. A generation has no source upload; an edit must retain the already verified `sourceUploadId`. `claimNextImageJob` returns that persisted operation and owner identity. `processNextImageJob` calls `generateGptImage` for generation, but for an edit it re-resolves the retained upload by tenant and user, downloads it only at worker time, and calls `editGptImage`. It must not reuse browser bytes, a URL, or a later preference. Migration `024_async_image_transform_jobs.sql`, documented in [schema](../data/schema.md), enforces this source/operation pairing; deploy it after the upload table and before the queued-edit code.
