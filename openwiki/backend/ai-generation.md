...
openwiki:
  ...
  invariants: [Document analysis produces storyboard scenes and a required recommended-style prompt, not editable presentation slides., Every analysis role resolves a tenant-assigned primary and optional fallback model or returns llm_not_configured rather than using environment configuration., API keys are encrypted at rest and never returned through management responses., Prompt and scene optimization emit English prose rather than a comma-delimited keyword list and preserve quoted literal in-image text in the requested language., Generation accepts creative inputs and assembles the final provider prompt on the server; it does not accept a browser-assembled prompt., Missing or unsupported image-language input adds no directive; none directs the model to render no text., GPT Image accepts only low medium or high quality at its public handlers; omitted or internal invalid values normalize to medium., A durable GPT image job retains its normalized quality through claiming and worker execution., The provider-neutral runtime makes at most four total attempts for retryable failures and retries recognized truncation on the same model with a larger budget.]
...
`POST /generate-images` is the server-owned text-to-image prompt boundary. It requires nonblank `userScript`, rather than accepting a browser-assembled `prompt`; optional `stylePrompt`, array `styleTags`, `purpose`, and `imageLanguage` remain separate creative inputs. `imagePrompt.js::buildImagePrompt` trims and de-duplicates nonblank style tags, keeps reusable style prose in a separate clause, and appends the subject, a composition directive, and the image-model language directive. It rejects empty content.

`normalizeImagePurpose` accepts `infographic`, `storyboard`, or `freeform` case-insensitively after trimming; all other or absent values fall back to `infographic`. The first two add presentation-slide or cinematic-storyboard composition guidance respectively. `freeform` adds neither that composition guidance nor an image-text-language directive, while retaining any supplied style prose, palette cues, and content description. The handler returns the resulting `prompt` for direct work and when it creates a durable job. Browser callers therefore must send inputs, not reproduce this composition or persist a guessed final prompt. `buildGenerationTextDirective` and optimizer-facing `buildImageTextDirective` share `IMAGE_TEXT_LANGUAGES`: for a supported language, quoted literal text remains exactly as authored and only other rendered text defaults to that language; `none` prohibits rendered text; and missing/unknown IDs add nothing.

The pure contract is covered by `imagePrompt.test.js`: purpose normalization/defaulting, style prose/tag separation and de-duplication, each composition behavior, text-language addition, missing content, transform-mode guarantees, and unknown-mode fallback. `aiService.test.js` is the narrow consumer serialization check. There is no handler test for the replacement request schema, tenant policy, returned prompt, or durable-job forwarding; add it before changing public validation or persistence behavior.
