...
`deckFrames.js` is the per-page geometry vocabulary. Each frame serves one or more page roles, declares a point-capacity range and non-overlapping modules within the 1280x720 safe area (except explicit bleed modules), and may declare an image module plus an image-free sibling. It is deliberately separate from the deck-wide `layout_id`: upstream template specifications set palette, typography, and narrative tone, while the selected frame owns page geometry.

`deckContract.js` is a cheap local preflight boundary, not a substitute for the Python gate. It normalizes outline slide numbers, page roles (`cover`, `toc`, `section`, `content`, `ending`), compatible frame choices, frame-defined point capacity, and zero-padded SVG filenames so sidecar ordering matches presentation order. It rejects malformed author output such as a wrong `1280x720` viewBox, missing/unknown page role, root transform, forbidden SVG constructs, named HTML entities, duplicate root-group IDs, or absent/out-of-canvas `data-pptx-bounds`.

`svgAuthoringPrompt.js` gives outline planning the frame catalog without coordinates, then gives each authoring and repair call only that slide's exact root-group IDs and bounds through `describeFrameGeometry`. It instructs repair to restore the frame before shrinking text; a template's layout prose cannot override the frame. `deckAuthor.js::authorDeck` generates each SVG, locally repairs preflight failures up to `DECK_MAX_REPAIR_ROUNDS` (3), writes it to the sidecar, and calls `saveDeckSlidePreview` through its `onSlidePreview` callback before the progress event, then calls `checkDeck`. The sidecar quality report decides whether the deck passes; rejected individual slides receive the gate's errors as repair feedback for up to three more whole-deck rounds. Each repaired SVG is saved again, incrementing that page's preview revision. `exportDeck` runs only after a passing report. Do not relax local checks to bypass an export failure: inspect the sidecar rule, frame, and prompt contract first.
...
- **Frame catalog, outline normalization, density reconciliation, or SVG grammar:** `deckFrames.js`, `deckContract.js`, `svgAuthoringPrompt.js`, and `deckAuthor.js`. Preserve role-compatible defaults, point-capacity truncation, safe-area/non-overlap geometry, frame-derived image role, sibling reconciliation, and the separation between deck-wide template tone and page geometry. `deckFrames.test.js` has retrievable suites for safe-area/non-overlap/catalog integrity and frame normalization; `deckContract.test.js` covers frame/image reconciliation as well as slide count, malformed outlines, deterministic filenames, and SVG preflight; `svgAuthoringPrompt.test.js` covers catalog versus chosen-frame prompt scope and repair ordering.
