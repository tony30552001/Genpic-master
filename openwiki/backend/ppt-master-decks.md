...
`processDeckJob` resolves the job tenant's `deck_authoring` assignment before outline authoring, then passes that Azure primary/fallback pair through `generateOutline` and every `authorSlideSvg`/repair call. This background SVG-authoring path accepts higher latency for stricter layout reasoning, but it no longer selects `AZURE_OPENAI_DECK_DEPLOYMENT` or any Azure deployment environment default. The shared adapter may retry retryable Azure Responses failures and switch to the assigned fallback; its budget and retry invariants are canonical in [AI generation](ai-generation.md). A missing role assignment is a terminal `llm_not_configured` job failure, not a queue retry. Configure model records and assignments on the Node/API administration surface only—not on the sidecar.
