...
openwiki:
  ...
  validation_commands: [pnpm test --run api/_shared/__tests__/azureEmbeddings.test.js, pnpm test --run api/history/__tests__/index.test.js api/admin/__tests__/adminResources.test.js src/components/admin/__tests__/AdminPanelSectionLoading.test.jsx]
---
...
`useHistory::saveHistoryItem` forwards source to `POST /history`. `InfographicGenerator` supplies `general` for ordinary image generation, `document` for a generated storyboard scene, and `image-transform` after an image transformation. History creation also requires a `jobId` for the same tenant/user whose image job has succeeded; the server takes the persisted job model and stores `history.image_job_id`, rather than trusting a browser model claim. The user history GET/POST response carries both source and job ID, so consumers should display null source as unrecorded rather than infer a workflow for pre-migration rows. The foreign key/provenance migration is in [schema](../data/schema.md), and the job lifecycle is in [AI generation](ai-generation.md).
...
For source changes, update the shared allowed list, the schema constraint/migration, every write call site, list serialization, admin query validation, and the UI labels together. `api/history/__tests__/index.test.js` covers job-backed history admission: valid source values, owner-scoped job lookup, persisted succeeded-job model, missing/malformed IDs, foreign-job concealment, and pending/failed-job rejection. `adminResources.test.js` exercises a recognized source, legacy `unknown`, and rejection of an unsupported admin filter; `AdminPanelSectionLoading.test.jsx` verifies source label rendering and request forwarding. The handler suite does not cover GET/DELETE serialization. Run:

```sh
pnpm test --run api/history/__tests__/index.test.js api/admin/__tests__/adminResources.test.js src/components/admin/__tests__/AdminPanelSectionLoading.test.jsx
```
