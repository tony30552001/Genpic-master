...
openwiki:
  ...
  source_paths: [db/migrations, db/migrations/009_image_generation_jobs.sql, db/migrations/010_auth_sessions.sql, db/migrations/011_deck_generation_jobs.sql, db/migrations/012_deck_job_events.sql, db/migrations/013_deck_slide_previews.sql, db/migrations/014_llm_models.sql, db/migrations/016_admin_list_indexes.sql, db/migrations/017_user_auth_provider.sql, db/migrations/018_history_source.sql, db/migrations/019_image_job_quality.sql, db/migrations/024_async_image_transform_jobs.sql, db/migrations/025_gpt_image_only.sql, db/migrations/026_image_model_catalog.sql, api/_shared/imageModels.js, api/_shared/modelPolicy.js, db/migrations/022_deck_recipe_brief.sql, db/migrations/023_deck_event_design_step.sql, api/_shared/imageJobs.js, api/_shared/gptImage.js, api/_shared/historySource.js, api/_shared/identity.js, api/_shared/llmProviders.js, api/_shared/llmModels.js, api/_shared/llmRuntime.js, api/_shared/deckJobs.js, api/_shared/uploads.js, api/_shared/documentAnalysisJobs.js, api/_shared/deckRecipes.js, api/scripts/migrate.cjs]
  invariants: ["The migration runner executes the explicitly selected or all .sql files in lexicographic order and has no applied-migration ledger.", "An optional migrator argument must exactly name an existing .sql migration; selected files still execute in lexicographic order.", "026_image_model_catalog.sql is a one-time, non-idempotent migration; existing environments must select only unapplied files and must never replay 025_gpt_image_only.sql over configured policy.", "Catalog-backed image jobs persist one of low medium high xhigh max or auto after validating the selected model capabilities.", "Image jobs are generate or edit operations; only edits retain a restrict-delete source upload, and history records successful owner job provenance.", "An image model is tenant-scoped with an immutable model key/API type/deployment identity; its encrypted key never belongs in a public projection.", "A deck job with images pins its selected image model key at admission.", "Authentication session records store token hashes, never raw browser tokens.", "A user's nullable last authentication provider is constrained to entra or google, backfilled from its newest session, and updated only after a successful active sign-in.", "History source is null or one of general document image-transform, and legacy rows retain null rather than a guessed source.", "Each analysis role assignment uses tenant-local primary and optional distinct fallback models; provider choice is a runtime dispatch concern.", "Deck jobs are scoped by both tenant and user and have only queued, processing, succeeded, or failed states.", "Deck image density is persisted as none, key, or every and defaults to key.", "Deck jobs persist recipe_id as general by default and retain optional purpose audience and outcome brief fields; recipe acceptance remains application normalization rather than a SQL check.", "Deck job events are append-only, constrained to source, outline, design, images, slides, quality, or export plus known statuses, and cascade with their job.", "Deck slide previews have one row per job and page; a quality rewrite increments revision and deletion of the job cascades."]
  validation_commands: [node api/scripts/migrate.cjs 026_image_model_catalog.sql, pnpm test --run api/_shared/__tests__/imageCatalog.integration.test.js]
---

# PostgreSQL schema and migrations

Migrations in `db/migrations` are the schema authority. With no arguments, `api/scripts/migrate.cjs` sorts and executes every `.sql` file; it has no applied-migration ledger. Supplying one or more exact migration filenames limits that run to the selected files, still in lexicographic order; an unknown filename exits with an error. Operators of an existing environment must therefore select **only unapplied** files. In particular, `025_gpt_image_only.sql` rewrites policy to GPT Image 2 and must never be replayed over configured policy; `026_image_model_catalog.sql` is a one-time, non-idempotent evolution, not a repeatable migration.
...
`019_image_job_quality.sql` historically added `image_generation_jobs.quality` as non-null text with a `medium` default and a check limited to `low`, `medium`, or `high`. `026_image_model_catalog.sql` replaces that check with the current six-value set: `low`, `medium`, `high`, `xhigh`, `max`, and `auto`. The catalog/model capability boundary now validates that a persisted value is supported by the selected model. `api/_shared/imageJobs.js::createImageJob` stores the selected value, and its claim/worker path forwards the persisted value to `gptImage.js`; this prevents a later browser preference change from affecting queued work. The provider and handler contract is canonical in [AI generation](../backend/ai-generation.md).
...
### Tenant image-model catalog

`026_image_model_catalog.sql` adds tenant-scoped `image_models`: immutable composite key `(tenant_id, model_key)`, unique normalized tenant label, Azure Images v1 API type, deployment/endpoint, encrypted API-key ciphertext, capabilities/default quality, audit users, and timestamps. SQL permits only `low`, `medium`, `high`, `xhigh`, `max`, and `auto`; the default must be supported. It also expands the image-job quality check to those values, makes new `tenant_model_settings` unconfigured (empty allow-list and nullable default), adds `history.image_job_id` with restrict-delete provenance, and stores/indexes `deck_generation_jobs.image_model_key` and active jobs by model.

The migration does not seed a catalog record or automatically select a policy: controlled rollout requires a tenant administrator to add models and set an allow-list/default before normal generation is admitted. Apply `026` after the migrations it alters and before deploying catalog-aware code. Check it only against a disposable database:

```sh
node api/scripts/migrate.cjs 026_image_model_catalog.sql
```

`imageCatalog.integration.test.js` can apply `026` in an isolated schema only when `TEST_DATABASE_URL` is set; it has not run here because Docker Engine was unavailable. The application lifecycle, encrypted key boundary, and attempt-fenced jobs are canonical in [AI generation](../backend/ai-generation.md).
...
`api/_shared/__tests__/imageCatalog.integration.test.js` applies `026_image_model_catalog.sql` in an isolated PostgreSQL schema and covers policy preservation/new-tenant initialization, encrypted catalog resolution, quality constraints, history references, and active-model deletion guards. It is gated by `TEST_DATABASE_URL` and was skipped locally because Docker Engine was unavailable. For a newly deployed migration, select only its exact unapplied filename—for example `node api/scripts/migrate.cjs 026_image_model_catalog.sql`—rather than replaying the directory. Do not use full-directory replay as ordinary validation: the runner has no ledger and replaying `025_gpt_image_only.sql` overwrites configured policy. For session changes, run `api/_shared/__tests__/session.test.js` and `auth.test.js`; for resource ownership, use [resources](../backend/resources.md) to identify handler checks. Production database credentials are never needed for ordinary validation.
