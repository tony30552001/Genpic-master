...
openwiki:
  ...
  validation_commands: [node api/scripts/migrate.cjs 025_gpt_image_only.sql, cd api && npm run migrate]
---
...
### Sole GPT Image policy

`025_gpt_image_only.sql` makes `gpt-image-2` the default and only allowed value for `tenant_model_settings`, then rewrites every existing policy whose default or allowed array differs. This migration prevents an existing tenant from selecting the removed Gemini renderer after `modelPolicy.js` and `imageProviders.js` drop it. Apply `025` after the tenant model-settings table exists and before deploying the only-renderer application code; it does not alter the separate tenant analysis-model catalog, where Google Gemini remains a valid text/structured-AI provider. Check the data update only against a disposable database:

```sh
node api/scripts/migrate.cjs 025_gpt_image_only.sql
```
