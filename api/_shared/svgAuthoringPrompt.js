...
- 一般直屬根層群組的 data-pptx-bounds 必須彼此分離；水平與垂直交集都超過 1 時會被拒絕。
- 文字需要壓在色塊、圖片或裝飾上時，請把承載物與文字放進同一個直屬根層 <g>，不要建立兩個互相重疊的根層群組。
- data-pptx-role 只用於真正的結構元素：background、chrome、decoration、footer、header、logo、page-number、watermark。一般卡片、內容底板或圖片不可標成結構角色來逃避重疊檢查。
...
- 檢查器估算的文字外框是：上緣 = y - 0.85 × font-size，下緣 = y + 0.35 × font-size，
  ...
  西文與半形字元平均約 0.55 × font-size。為 PowerPoint 換行預留空間時，再將整行估算寬度乘以 1.06。
...
2. 一般根層群組不得互相重疊；文字與承載底板需要疊放時，合併到同一個根層群組。
3. 再把每個群組內的文字排回它自己的框內：調整 x／y、改用 <tspan> 換行、或收斂行距。
4. 只有在文字量確實超過該框的容量時，才精簡文字。
5. 其餘設計（色彩、字體、層級）保持不變。`
    : `1. 先針對錯誤訊息指出的那幾個群組調整幾何：移動位置、調整 data-pptx-bounds、或改變區塊的分配方式。
2. 一般根層群組不得互相重疊；文字與承載底板需要疊放時，合併到同一個根層群組。
3. 再把該群組內的文字排回它自己的框內：調整 x／y、改用 <tspan> 換行、或收斂行距。
4. 只有在文字量確實超過可用空間時，才精簡文字 —— 刪內容是最後手段，不是第一步。
5. 其餘設計（色彩、字級、網格對齊、幾何語彙）保持不變，不要藉修正之便重做整頁。`;
